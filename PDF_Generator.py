...
def create_combined_pdf(output_json_path, quality_analysis_json_path, output_pdf_path):
    with open(output_json_path, 'r') as fp:
    ...
    title = Paragraph(f"<para alignment='center'><b>{name}</b><br/></para>" f"<para alignment='center'>{formatted_date}</para>", styles['Title'])
    ...
    try:
        with open(quality_analysis_json_path, 'r') as fp:
